        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <form action="{{ route('sub-criteria.destroy', $subCriteria->id) }}" method="POST" class="d-inline m-0">
                        @csrf
                        @method('delete')
                        <button type="submit" class="btn text-danger m-0 p-0"><i class="fas fa-trash"></i></button>
                    </form>
                    <br>
                    @endif
                    @endforeach
                ...
                <td width="290px">
                    <form action="{{ route('criteria.destroy', $criteria->id) }}" method="POST">
                        @csrf
                        @method('delete')
                        <a href="{{ route('criteria.show', $criteria->id) }}" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Sub Kriteria</a>
                        <a href="{{ route('criteria.edit', $criteria->id) }}" class="btn btn-success d-inline-block"><i class="fas fa-edit"></i></a>
                        <button type="submit" class="btn btn-danger d-inline-block"><i class="fas fa-trash"></i></button>
                    </form>
                </td>
            </tr>
            @endforeach
